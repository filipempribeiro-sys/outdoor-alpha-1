...
const CHAT_FULLSCREEN_SIDEBAR_FIX_URL="./olen-4.4.0-chat-sidebar-fullscreen-fix.js?v=4.5.2-final-5of5-20260914";
const NAV_STARTUP_FIX_URL="./olen-4.4.0-navigation-startup-fix.js?v=4.4.0-nav-startup-20260913";
...
const SHELL=[
  ...
  "./assets/olen/olen-ring.png","./assets/olen/olen-star.png","./assets/olen/olen-chevron.png","./assets/olen/olen-text.png",
  ...
  "./olen-4.4.0-identity.js","./olen-4.4.0-splash-guard.js","./olen-4.4.0-splash-polish.js","./olen-4.4.0-chat-sidebar-fullscreen-fix.js","./olen-4.4.0-navigation-startup-fix.js",
...
function with440Patch(response){
 ...
 return response.text().then(html=>{
  const tags=[BASE_PATCH_URL,SPOTIFY_PATCH_URL,CALENDAR_GOOGLE_URL,CALENDAR_TIME_URL,PLACE_EXPERIENCE_URL,PLACE_CLEANUP_URL,CHAT_SIDEBAR_URL,CHAT_SIDEBAR_FIX_URL,CHAT_FOOTER_ICONS_URL,HOME_BUTTONS_ONLY_URL,CHAT_FOOTER_IMMEDIATE_URL,CHAT_SIDEBAR_FULL_SCROLL_URL,CHAT_SIDEBAR_FIXED_ENDS_URL,CHAT_FOOTER_ICONS_LOCK_URL,HOME_NO_SPOTIFY_URL,HOME_FOOTER_SIX_GRID_URL,INTERNAL_SIDEBAR_NO_SPOTIFY_URL,OLEN_IDENTITY_URL,SPLASH_GUARD_URL,SPLASH_POLISH_URL,HOME_UX_POLISH_URL,NAV_STARTUP_FIX_URL,CHAT_FULLSCREEN_SIDEBAR_FIX_URL].map(u=>'<script src="'+u+'"><\/script>').join('');
  const bootStyle='<style id="olenBootFirstPaint440">html.olenBootPending440 body{background:#050b10!important}html.olenBootPending440 body>*:not(#alphaWelcomeOverlay){visibility:hidden!important}</style><script>document.documentElement.classList.add("olenBootPending440")<\/script>';
  ...
  html=html.replace(/<script src="\.\/alpha-4\.3\.17-fixes\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\.\/alpha-4\.3\.19-chat-spotify\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\.\/alpha-4\.3\.22-spotify-global\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\.\/alpha-4\.3\.30-(?:calendar-google|calendar-timepicker|place-experience|place-detail-cleanup|media-bridge|chat-sidebar-stage3|chat-sidebar-polish|chat-sidebar-stage3-fix2)\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\.\/alpha-4\.3\.(?:32-chat-footer-icons-only|33-home-buttons-only|34-chat-footer-immediate|35-sidebar-full-scroll|36-sidebar-fixed-header-footer|37-sidebar-footer-icons-lock|38-home-no-spotify-footer|39-home-footer-six-grid|40-internal-sidebar-no-spotify|41-internal-sidebar-cleanup|43-home-ux-polish)\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\.\/olen-4\.4\.0-(?:identity|splash-guard|splash-polish|chat-sidebar-fullscreen-fix|navigation-startup-fix)\.js\?v=[^"]+"><\/script>/g,'').replace(/<style id="alphaSpotifyExactAlign4329">[\s\S]*?<\/style>/g,'').replace(/<style id="olenBootFirstPaint440">[\s\S]*?<\/style><script>document\.documentElement\.classList\.add\("olenBootPending440"\)<\/script>/g,'');
  if(html.includes('<head>'))html=html.replace('<head>','<head>'+bootStyle);else html=bootStyle+html;
  const allTags=alignStyle+tags;html=html.includes('</body>')?html.replace('</body>',allTags+'</body>'):html+allTags;
  html=html.replace(/<meta name="alpha-version" content="[^"]*">/,'<meta name="alpha-version" content="4.4.0">').replace(/<title>Project Alpha [^<]*<\/title>/,'<title>OLEN 4.4.0 — Outdoor Lifestyle Experience</title>').replace(/const APP_VERSION='4\.3\.16'/g,"const APP_VERSION='4.3.43'").replace(/const ALPHA_VISIBLE_VERSION='v4\.3\.16'/g,"const ALPHA_VISIBLE_VERSION='v4.3.43'").replace(/window\.ALPHA_DOMAIN_GATE_BUILD='4\.3\.16-full-system-hardening'/g,"window.ALPHA_DOMAIN_GATE_BUILD='4.3.43'").replace(/A iniciar a ALPHA(?:\s*4\.3\.(?:18|43))?(?:…|\.\.\.)/g,'A iniciar a OLEN…').replace(/PROJECT\s+ALPHA/g,'OLEN').replace(/\bALPHA\b/g,'OLEN').replace(/\bAlpha\b/g,'OLEN');
  const headers=new Headers(response.headers);headers.delete('content-length');headers.set('Cache-Control','no-store');headers.set('X-OLEN-Patch','4.5.2-chat-final-5of5-20260914');return new Response(html,{status:response.status,statusText:response.statusText,headers});
 })
}
...
self.addEventListener('fetch',e=>{
 ...
 if(u.origin===location.origin&&u.pathname.endsWith('/olen-4.4.0-chat-sidebar-fullscreen-fix.js')){e.respondWith(fetch(e.request,{cache:'reload'}).then(r=>{const cp=r.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,cp)).catch(()=>{});return r}).catch(()=>caches.match(e.request)));return}
