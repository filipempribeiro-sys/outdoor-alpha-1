const CACHE_NAME="olen-v4.4.0-navigation-startup-fix-20260913";
...
function with440Patch(response){
 ...
 return response.text().then(html=>{
  ...
  const headers=new Headers(response.headers);headers.delete('content-length');headers.set('Cache-Control','no-store');headers.set('X-OLEN-Patch','4.4.0-navigation-startup-fix-20260913');return new Response(html,{status:response.status,statusText:response.statusText,headers});
 })
}
