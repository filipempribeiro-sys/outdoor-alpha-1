...
(function(global){
'use strict';
if(global.OLEN5?.productionEntry?.version==='5.0.0')return;
...
function queryEnabled(){
  try{return new URLSearchParams(global.location.search).get('olen5')==='1'}catch{return false}
}
...
function loadLoader(){
  ...
  return new Promise((resolve,reject)=>{
    ...
    if(el){
      el.addEventListener('load',()=>resolve(global.OLEN5?.runtimeLoader),{once:true});
    ...
    el.src=LOADER_SRC;
    el.async=false;
    el.dataset.olen5ProductionLoader='true';
    el.addEventListener('load',()=>{
      if(!global.OLEN5?.runtimeLoader)return reject(new Error('olen5_runtime_loader_contract_missing'));
      resolve(global.OLEN5.runtimeLoader);
    },{once:true});
    el.addEventListener('error',()=>reject(new Error('olen5_runtime_loader_failed')),{once:true});
...
function enablePersisted(){
  try{global.localStorage.setItem(STORAGE_KEY,'1');return true}catch{return false}
}

function disablePersisted(){
  try{global.localStorage.removeItem(STORAGE_KEY);return true}catch{return false}
}
...
function rollback(reason='production-entry-rollback'){
  ...
  document.documentElement.removeAttribute('data-olen5-loaded');
...
global.OLEN5.productionEntry=api;
