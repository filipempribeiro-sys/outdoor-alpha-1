/* ALPHA 4.3.31b · CALENDAR GOOGLE SYNC
   Google OAuth + real event sync + encrypted persistent grant.
   Compact month grid, hidden provider scrollbar and explicit sync diagnostics.
*/
(()=>{
'use strict';
if(window.__alphaCalendarGoogle431b)return;window.__alphaCalendarGoogle431b=true;
...
function explainError(e){
  const code=String(e?.code||'');if(code==='GOOGLE_CALENDAR_SCOPE')return 'O Google não concedeu a permissão necessária para ler os eventos. Toca em Google e volta a autorizar.';
...
async function status(){
  try{const d=await json(`${BACKEND}/api/google/calendar/status?sessionId=${encodeURIComponent(sessionId)}`);googleConnected=Boolean(d.connected);googleEmail=String(d.email||'');providerButton();enableGoogleOption();if(googleConnected)await syncGoogle(true)}
  catch(e){console.warn('[ALPHA Calendar] status',e);setProviderError(e)}
}
async function connectGoogle(){
  try{setProviderBusy('Google · a ligar…');const returnTo=location.href.split('#')[0];const d=await json(`${BACKEND}/api/google/calendar/auth-url?sessionId=${encodeURIComponent(sessionId)}&returnTo=${encodeURIComponent(returnTo)}`);location.href=d.url}
  catch(e){console.error('[ALPHA Calendar] auth',e);setProviderError(e);window.toast?.('Não foi possível iniciar a ligação ao Google Calendar.')}
}
...
async function syncGoogle(force=false,retry=true){
  ...
  try{const d=await json(`${BACKEND}/api/google/calendar/events?sessionId=${encodeURIComponent(sessionId)}&timeMin=${encodeURIComponent(w.timeMin)}&timeMax=${encodeURIComponent(w.timeMax)}`);saveGoogle(d.events||[]);lastSyncKey=w.key;lastError='';providerButton();renderNoSync()}
    ...
    console.warn('[ALPHA Calendar] sync',e.code,e.status,e.reason,e.message);
    if(retry&&(!e.status||e.status>=500)){syncing=false;await new Promise(r=>setTimeout(r,1200));return syncGoogle(true,false)}
    if(/NOT_CONNECTED|SESSION_EXPIRED|TOKEN|401/.test(String(e.code||'')+' '+String(e.message||''))){googleConnected=false;saveGoogle([]);providerButton();enableGoogleOption()}
    else setProviderError(e);
    window.toast?.(explainError(e));
  }finally{syncing=false}
}

const nativeRender=typeof window.alphaCalendarRender==='function'?window.alphaCalendarRender:null;
function compactGrid(){
  const grid=$('alphaCalendarGrid');if(!grid)return;const c=window.alphaCalendarCursor instanceof Date?window.alphaCalendarCursor:new Date(),y=c.getFullYear(),m=c.getMonth(),first=new Date(y,m,1),offset=(first.getDay()+6)%7,days=new Date(y,m+1,0).getDate(),needed=Math.ceil((offset+days)/7)*7;
  [...grid.children].forEach((el,i)=>{if(i>=needed)el.remove()});
  [...grid.querySelectorAll('.alphaCalendarDay')].forEach(day=>{const iso=day.getAttribute('onclick')?.match(/'([^']+)'/)?.[1];if(!iso)return;const events=window.alphaCalendarEvents?.().filter(e=>e.date===iso)||[],dots=[...day.querySelectorAll('.alphaCalendarDot')];events.slice(0,dots.length).forEach((e,i)=>{if(e.provider==='google')dots[i].classList.add('google')})});
}
...
function enableGoogleOption(){const sel=$('alphaCalProvider');if(!sel)return;const opt=[...sel.options].find(o=>o.value==='google');if(opt){opt.disabled=!googleConnected;opt.textContent=googleConnected?'Google Calendar':'Google · ligar primeiro'}}
...
const nativeOpen=window.alphaCalendarOpenEditor;if(typeof nativeOpen==='function')window.alphaCalendarOpenEditor=function(id=''){nativeOpen(id);enableGoogleOption()};
...
if(typeof nativeSaveEditor==='function')window.alphaCalendarSaveEditor=async function(){
  ...
  const body={sessionId,title,description:$('alphaCalNotes')?.value.trim()||'',location:$('alphaCalLocation')?.value.trim()||'',...range};
  try{const remoteId=id.startsWith('gcal:')?id.slice(5):'';if(remoteId)await json(`${BACKEND}/api/google/calendar/events/${encodeURIComponent(remoteId)}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});else await json(`${BACKEND}/api/google/calendar/events`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});window.alphaCalendarSelectedDate=date;window.alphaCalendarCursor=new Date(date+'T12:00:00');window.alphaCalendarCloseEditor?.();lastSyncKey='';await syncGoogle(true);window.toast?.(remoteId?'Evento atualizado no Google Calendar.':'Evento criado no Google Calendar.')}
...
function handleReturn(){
  const u=new URL(location.href),v=u.searchParams.get('alphaGoogleCalendar'),g=u.searchParams.get('alphaGoogleGrant');if(g)saveGrant(g);if(!v&&!g)return;
  u.searchParams.delete('alphaGoogleCalendar');u.searchParams.delete('alphaGoogleGrant');history.replaceState(null,'',u.pathname+u.search+u.hash);
  if(v==='connected')window.toast?.('Google Calendar ligado à ALPHA.');else if(v==='denied')window.toast?.('Ligação ao Google Calendar cancelada.');else if(v)window.toast?.('Não foi possível ligar o Google Calendar.');
}
...
function install(){style();handleReturn();providerButton();enableGoogleOption();renderNoSync();status();console.info('[ALPHA 4.3.31b] Calendar Google persistent frontend sync ativo')}
