/* ALPHA 4.3.32 · CHAT FOOTER ICONS ONLY
   Visual-only patch for the Chat sidebar footer.
   Replaces footer labels/generic fallbacks with explicit icons.
   No navigation, search, conversation, gesture or sidebar behavior is changed.
*/
(()=>{
'use strict';
if(window.__alphaChatFooterIcons432)return;window.__alphaChatFooterIcons432=true;
...
function sourceText(src){
 return [src?.id,src?.className,src?.getAttribute?.('aria-label'),src?.getAttribute?.('title'),src?.getAttribute?.('data-view'),src?.getAttribute?.('data-tab'),src?.getAttribute?.('data-page'),src?.getAttribute?.('data-route'),src?.getAttribute?.('data-target'),src?.textContent].filter(Boolean).join(' ').toLocaleLowerCase('pt-PT');
}
function iconFor(src,i,total){
 ...
 const fallback=total>=7
   ? [ICON.home,ICON.map,ICON.chat,ICON.calendar,ICON.live,ICON.reports,ICON.profile]
   : [ICON.home,ICON.map,ICON.chat,ICON.calendar,ICON.live,ICON.profile];
...
function apply(){
 ...
 buttons.forEach((b,i)=>{
   const src=native[i];
   if(!src)return;
   b.innerHTML=iconFor(src,i,buttons.length);
 });
}
window.addEventListener('pageshow',()=>setTimeout(apply,120),{passive:true});
document.addEventListener('click',()=>setTimeout(apply,0),true);
setTimeout(apply,140);
console.info('[ALPHA 4.3.32] chat footer icons-only patch ativo');
})();
