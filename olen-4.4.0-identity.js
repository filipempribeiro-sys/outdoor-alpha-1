...
function brandHome(){
 ...
 if(brand){brand.querySelector('.logo')?.classList.add('olen440-ui-logo');const b=brand.querySelector('b');if(b)b.textContent='OLEN';brand.querySelectorAll('small').forEach(s=>{s.style.display='none'})}
...
function brandChatSidebar(){
 ...
 const name=side.querySelector('.alphaChatOlenName4330');if(name)name.textContent='OLEN';
}
...
function brandInternalSidebar(){
 ...
 if(brand){const name=brand.querySelector('b');if(name)name.textContent='OLEN';let mark=brand.querySelector('.olen440-internal-logo');if(!mark){const candidate=[...brand.children].find(el=>el!==name&&!/^(B|STRONG)$/i.test(el.tagName));if(candidate){candidate.classList.add('olen440-internal-logo');mark=candidate}else if(name){mark=document.createElement('span');mark.className='olen440-internal-logo';mark.setAttribute('aria-hidden','true');name.before(mark)}}}
